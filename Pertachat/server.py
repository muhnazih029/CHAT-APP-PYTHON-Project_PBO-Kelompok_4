...
class Client:
    ...
    def run_gui(self):
        root = tk.Tk()
        root.title("Client Controller")
        root.protocol("WM_DELETE_WINDOW", self.on_end_server)

        label = tk.Label(root, text="Client Controller")
        label.pack()

        self.server_on = tk.Button(root, text="Start Server", command=klien.start_server_thread)
        self.server_on.pack()

        self.button = tk.Button(root, text="Add Client", command=self.on_button_click)
        self.button.pack()

        self.end_server_button = tk.Button(root, text="End Server", command=self.on_end_server)
        self.end_server_button.pack()

        self.status_label = tk.Label(root, text=f"Current Clients: {self.current_clients}/{self.max_clients}")
